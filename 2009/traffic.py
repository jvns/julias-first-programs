    ...
    def checkDanger(self, currTile, yieldTile, dist):
        ...
        for i in nextTile.prev:
            if i is not yieldTile:
                otherTile = i
        #otherTile = nextTile.prev[1]
        # should be 0 in other case
#        for tile in nextTile.prev:
#            if (currTile == tile):
#                continue
#            else: 
#                otherTile = tile
#                break
#        else: 
#            print "oh no! there is no previous tile"
#        print "My tile: ", currTile
#        print "Other tile: ", otherTile
        dist2 = 1
        while True:
            if (dist2 >= otherTile.track.length):
                return False
            if (otherTile.car):
#                print "My car: ", currTile.car
#                print "Other car: ", otherTile.car
                result = self.checkBackOne(dist, dist2, currTile.car.speed, otherTile.car.speed)
#                print result, dist, dist2, currTile.car.speed, otherTile.car.speed
                if (result == 0):
                    return True
                if (result == -1):
                    return False
#            print otherTile.prev
            otherTile = otherTile.prev[0] # TODO: wrong
            dist2 += 1

#        flags = [False]
#        if tile.car:
#            if tile.car.speed*(tile.car.speed -1)/2 > dist:
#                if dist == 0:
#                    flags.append(False)
#                else:
#                    flags.append(checkDanger(tile.next, dist-1))
#            else:
#                flags.append(True)
#        if prevTile in tile.yield_:
#            for t in tile.prev:
#                if t is not prevTile
#                flag.append(checkReverseDanger(t, speed))
#            
        
    def update(self):
        global numLeftThisTurn
#        print "Updating car at index %i" % self.tile.index
        # set nextLocation
...
#                print "checking for danger..."
                if self.checkDanger(self.tile, track[j], dist + 1):
                    doDecel = True
#                    print "decelerating because of DANGER"
            # wrong:
...
#                print "holy crap there should not be a car there"

            if stoppingDistance(self.speed) >= stoppingDistance(nextCar.speed - decel) + currDist:
                doDecel = True
#                print "decelerating because of car ahead"
        ...
        if (doDecel):
            ...
            for dist in range(0, stoppingDistance(self.speed)+ carLength):
                ...
                if (track[j] == None):
                    continue
                if (track[j].yieldTo != None):
#                    print "there is a yield sign!"
                    if self.checkDanger(self.tile, track[j], dist + 1):
#                        print "there is danger!"
#                        print "my speed is %i" % self.speed
                        self.speed = oldspeed
                    break
...
def update():
    for car in cars:
        car.tile.car = car # should not be here
#        print "My speed: ", car.speed
        car.update()
